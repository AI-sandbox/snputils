# Ancestry

Objects, readers, and writers for local and global ancestry data.

## Objects

```{eval-rst}
.. autoclass:: snputils.LocalAncestryObject
   :members:
```

```{eval-rst}
.. autoclass:: snputils.GlobalAncestryObject
   :members:
```

## Readers

```{eval-rst}
.. autoclass:: snputils.MSPReader
   :members:
```

```{eval-rst}
.. autoclass:: snputils.FLAREReader
   :members:
```

```{eval-rst}
.. autoclass:: snputils.LANCReader
   :members:
```

```{eval-rst}
.. autoclass:: snputils.AdmixtureReader
   :members:
```

## Read Functions

```{eval-rst}
.. autofunction:: snputils.read_lai
```

```{eval-rst}
.. autofunction:: snputils.read_msp
```

```{eval-rst}
.. autofunction:: snputils.read_flare
```

```{eval-rst}
.. autofunction:: snputils.read_lanc
```

```{eval-rst}
.. autofunction:: snputils.read_adm
```

```{eval-rst}
.. autofunction:: snputils.read_admixture
```

## Writers

```{eval-rst}
.. autoclass:: snputils.MSPWriter
   :members:
```

```{eval-rst}
.. autoclass:: snputils.FLAREWriter
   :members:
```

```{eval-rst}
.. autoclass:: snputils.LANCWriter
   :members:
```

```{eval-rst}
.. autoclass:: snputils.AdmixtureWriter
   :members:
```

```{eval-rst}
.. autoclass:: snputils.AdmixtureMappingVCFWriter
   :members:
```
