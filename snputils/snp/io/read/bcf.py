...
class BCFReader(SNPBaseReader):
            ...
            if n_file_samples == 0 and np.all(n_fmt_arr == 0):
                ...
                if uniform_indiv:
                    ...
                    for i in range(n_records):
                        ...
                        if sum_strands:
                            missing_as_haploid = (
                                non_diploid_chromosomes is not None
                                and bool(non_diploid_chromosomes[i])
                            )
    ...
    def _try_read_gt_only_all(
        ...
        if sum_strands and _header_has_non_diploid_contigs(header):
            return None
    ...
    def _read_filtered(
        ...
        for out_idx, record_offset in enumerate(record_offsets_list):
            ...
            missing_as_haploid = (
                sum_strands
                and _normalized_non_diploid_chromosome(header.contigs[contig_id]) is not None
            )
